...
class cl_widgets :
  def __init__( self, ob_root_window, ob_world = [] ) :
    ...
    self.m_Euler = tk.BooleanVar()
    self.m_Euler.set( False )
...
class cl_menu :
  def __init__( self, master ) :
    ...
    dummy.add_checkbutton( label = 'Perspective', variable = self.master.m_Perspective )
    dummy.add_checkbutton( label = 'Euler', variable = self.master.m_Euler )
...
class cl_toolbar :
  ...
  def resolution_callback( self ) :
    f = simpledialog.askinteger( "Resolution", "Value?",
              initialvalue = self.master.resolution , minvalue = 4.0 )
  ...
  def phi_callback( self ) :
    p = simpledialog.askfloat( "φ", "Value?",
              initialvalue = self.master.phi , minvalue = -180.0, maxvalue = 180.0 )

    if ( p is None ) :
        msg =  "[Angle was cancelled]"  
    else :
        self.master.phi = p
        msg = f'φ  {p}°'
  ...
  def theta_callback( self ) :
    t = simpledialog.askfloat( "θ", "Value?",
              initialvalue = self.master.theta , minvalue = -180.0, maxvalue = 180.0 )
  ...
  def psi_callback( self ) :
    ps = simpledialog.askfloat( "ψ", "Value?",
              initialvalue = self.master.psi , minvalue = -180.0, maxvalue = 180.0 )
  ...
  def distance_callback( self ) : 
    f = simpledialog.askfloat( "Distance", "Value?",
              initialvalue = self.master.m_Distance , minvalue = 1.0 )

    if ( f is None ) :
        msg =  "[Distance was cancelled]"  
    else :
  ...
  def draw_callback( self ) :
    ...
    self.master.m_ModelData.specifyEuler( self.master.phi, self.master.theta, self.master.psi )
    ...
    self.master.ob_world.create_graphic_objects( self.master.ob_canvas_frame.canvas, self.master.m_ModelData, clip, perspective, euler, resolution )
